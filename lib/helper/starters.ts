...
import {
  ...
  resolveHardhatTemplateDir,
  resolveMarkdownTemplateDir,
  resolveOverridesTemplateDir,
  resolveStarterDir,
  resolveStarterMetadataFile,
  resolveStartersDir,
  resolveUiStarterDir,
  resolveWorkspaceStarterDir,
} from "./path-utils";
...
import {
  ReadmeContractEntry,
  ReadmeTemplateData,
} from "../types/markdownFile.schema";
...
function readAdditionalPackages(sourceDir: string): AdditionalPackages {
  ...
  if (!parsed.ok) {
    logger.warning(
      `Gagal membaca packages.extra.json di ${quotePath(manifestPath)}: ${parsed.error}`,
    );
    return {};
  }
...
export async function getAllStarters(
  returnAbsolutePaths: boolean = false,
): Promise<string[]> {
 ...
 */
export async function checkStarterExists(
  starterName: string,
): Promise<boolean> {
  ...
  for (const name of starterNames) {
    ...
    if (fs.existsSync(metadataFile)) {
      ...
      if (metadata.name !== name) {
        logger.warning(
          `Metadata name mismatch for starter: ${name} (found: ${metadata.name})`,
        );
        continue;
      }
 ...
 */
export async function getStarterMetadataField(
  starterName: string,
  field: string | string[],
...
export function handleSkipUi(skipUi: boolean | undefined): void {
  ...
  if (!skipUi) {
    ...
    if (!fs.existsSync(builtUiDir)) {
      logger.error(
        `Direktori dist tidak ditemukan di template frontend: ${quotePath(builtUiDir)}`,
      );
      logger.error(
        `Silakan jalankan ${quotePath("npm start")} atau ${quotePath("npm run template:build-ui")} dari root project terlebih dahulu.`,
      );
      process.exit(1);
    }
  }
 ...
 */
export async function copyStarterToWorkspace(
  starterNames: string[],
  ...
  for (const starterName of starterNames) {
    ...
    if (extraPackages.scripts) {
      Object.entries(extraPackages.scripts).forEach(([k, v]) =>
        scriptsMap.set(k, v),
      );
    }
    ...
    if (!skipUi) {
      ...
      const targetUiMetadataDir = path.join(
        targetDir,
        "ui",
        "contracts",
        starterName,
      ...
      const targetUiMetadataFile = path.join(
        targetUiMetadataDir,
        "metadata.json",
      );
      logger.info(
    ...
    contractList.push({
      ...
      name: (await getStarterMetadataField(starterName, "contract_name"))
        .contract_name,
      file: (await getStarterMetadataField(starterName, "contract_filename"))
        .contract_filename,
    });
  ...
  fs.writeFileSync(contractListFile, JSON.stringify(contractList, null, 2), {
    encoding: "utf-8",
  });
  ...
  const hasDevDeps = devDeps.length > 0;
  const scripts =
    scriptsMap.size > 0 ? Object.fromEntries(scriptsMap.entries()) : undefined;
 ...
 */
export function generateWorkspaceReadmeFromStarters(
  fileName: string,
  destinationDir: string,
  starterNames: string[],
): void {
  ...
  for (const starterName of starterNames) {
    ...
    if (fs.existsSync(metadataFile)) {
      ...
      logger.warning(
        `Metadata file not found for starter: ${starterName}. README will contain default placeholders.`,
      );
      contracts.push({
        name: starterName,
        ...
        details:
          "This is a placeholder. Update metadata.json to populate contract details.",
      });
    }
  }
  ...
  const templateContent = renderHbsFile<ReadmeTemplateData>(sourceFile, {
    ...
    title: `FHEVM StarterKit Project (${destinationDir
      .split("-")
      .map((w) => w.charAt(0).toUpperCase() + w.slice(1))
      .join(" ")})`,
    description:
      "This project is a self-contained learning unit generated from FHEVM StarterKit.",
