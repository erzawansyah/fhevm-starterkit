 ...
 * @notice Demonstrates public, permissionless decryption of a single encrypted boolean using FHE.makePubliclyDecryptable.
 * @dev Usage summary:
 * - Call headsOrTails(headsPlayer, tailsPlayer) to start a game and emit a decryptable ebool handle
 * - Use the relayer to run publicDecrypt(handle) to obtain the clear value and a proof
 * - Call recordAndVerifyWinner(gameId, abiEncodedClearGameResult, decryptionProof) to verify and store the winner
...
contract PublicDecryptSingleValue is ZamaEthereumConfig {
    ...
    struct Game {
        ...
        address tailsPlayer;
        /// @notice Publicly decryptable encrypted game result.
    ...
    function headsOrTails(address headsPlayer, address tailsPlayer) external {
        ...
        // Make the result publicly decryptable so anyone can request decryption via the relayer.
    ...
    function recordAndVerifyWinner(
        ...
        // Creating the list of handles (only one here). Order is trivial for a single handle.
        ...
        //    In this specific case, the function argument `abiEncodedClearGameResult` could have been a simple
        //    `bool` instead of an ABI-encoded bool; the caller would then compute abi.encode on-chain.
