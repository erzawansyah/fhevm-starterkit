...
import { PublicDecryptSingleValue, PublicDecryptSingleValue__factory } from "../../../typechain-types";
...
describe("PublicDecryptSingleValue", function () {
  ...
  it("decryption should succeed", async function () {
    ...
    // Winner should not be retrievable before verification
    await expect(contract.getWinner(gameId)).to.be.revertedWith("Game winner not yet revealed");
    ...
    // Duplicate reveal should fail
    await expect(contract.recordAndVerifyWinner(gameId, abiEncodedClearGameResult, decryptionProof)).to.be.revertedWith(
