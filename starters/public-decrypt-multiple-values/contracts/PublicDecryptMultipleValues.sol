 ...
 * @notice Demonstrates a public, permissionless 8-sided die roll using FHE.makePubliclyDecryptable.
 * @dev Usage summary:
 * - Call highestDieRoll(playerA, playerB) to start a game and emit decryptable handles
 * - Ask the relayer to run publicDecrypt on both handles to obtain clear values and a proof
 * - Call recordAndVerifyWinner(gameId, abiEncodedClearGameResult, decryptionProof) to persist the winner on-chain
...
contract PublicDecryptMultipleValues is ZamaEthereumConfig {
     ...
     * @param playerA The address of the first player.
     * @param playerB The address of the second player.
     */
    function highestDieRoll(address playerA, address playerB) external {
        ...
        // Make the results publicly decryptable so anyone can request decryption via the relayer.
    ...
    function recordAndVerifyWinner(
        ...
        // Creating the list of handles in the correct order; the verification step is order-sensitive.
