...
import {
  ...
  getFilteredStarter,
  getAllStarterMetadata,
  copyStarterToWorkspace,
  generateWorkspaceReadmeFromStarters,
} from "../../lib/helper/starters";
 ...
 */
async function resolveActionType(
  starterNames: string[],
  dir?: string,
  ...
  if (starterNames.length === 1) {
    ...
    if (dir) {
      ...
      const answer = await prompt<{ name: string }>({
        ...
        name: "name",
        message:
          "You are creating multiple starters. Enter the workspace directory:",
...
function resolveFetchMode(opts: StarterCreateOptions): StarterCreateFetchMode {
  ...
  if (starterNames && starterNames.length > 0) {
    ...
    if (
      ...
      logger.error(
        "Cannot use positional starter names with filter options (category, chapter, tags, concepts). Please choose one mode of selection.",
      );
      process.exit(1);
    }
    return "positional";
 ...
 */
async function handlePositionalMode(
  opts: StarterCreateOptions,
): Promise<StarterCreatePlan> {
  ...
  if (unAvailableStarters.length > 0) {
    logger.error(
      `The following starter(s) do not exist: ${unAvailableStarters.join(", ")}`,
    );
    process.exit(1);
  }
  ...
  const { destinationDir, destinationPath, actionType } =
    await resolveActionType(starterNames, opts.dir);
  return {
    mode: "positional",
    starterNames,
    destinationDir,
 ...
 * @returns Promise<StarterCreatePlan>
 */
async function handleFilterMode(
  opts: StarterCreateOptions,
): Promise<StarterCreatePlan> {
  ...
  if (category) {
    ...
    logger.info(
      `Found ${filteredStarters.length} starter(s) matching category "${category}".`,
    );
  ...
  if (chapter) {
    ...
    logger.info(
      `Found ${filteredStarters.length} starter(s) matching chapter "${chapter}".`,
    );
  ...
  if (tags && tags.length > 0) {
    ...
    logger.info(
      `Found ${filteredStarters.length} starter(s) matching tags "${tags.join(", ")}" with operator "${and ? "and" : "or"}".`,
    );
    filtered["tags"] = filteredStarters;
  }
  if (concepts && concepts.length > 0) {
    const filteredStarters = await getFilteredStarter(
      "concepts",
      concepts.join(","),
    );
    logger.info(
      `Found ${filteredStarters.length} starter(s) matching concepts "${concepts.join(", ")}" with operator "${and ? "and" : "or"}".`,
    );
  ...
  if (!and) {
    ...
    const filteredArrays = Object.values(filtered).filter(
      (arr) => arr.length > 0,
    );
  ...
  logger.info(
    `Total starters selected after applying filters: ${combinedStarters.length}`,
  );
  logger.info(`Starters: ${combinedStarters.join(", ")}`);

  const { actionType, destinationDir, destinationPath } =
    await resolveActionType(combinedStarters, opts.dir, true);
...
async function handleInteractiveMode(): Promise<StarterCreatePlan> {
  ...
  for (const meta of allMetadata) {
    ...
    if (Array.isArray(meta.tags))
      meta.tags.forEach((t) => tagSet.add(String(t)));
    if (Array.isArray(meta.concepts))
      meta.concepts.forEach((c) => conceptSet.add(String(c)));
  }
  ...
  const entry = await prompt<{
    ...
    choices: [
      ...
      {
        name: "filter",
        message: "Filter by taxonomy (category/chapter/tags/concepts)",
      },
    ],
  });
  ...
  if (entry.mode === "pick") {
    ...
    const selection = await prompt<{ starters: string[] }>({
      ...
      name: "starters",
      message:
        "Select starter(s) to create (Press <space> to select, <a> to toggle all, <i> to invert selection):",
      choices,
    });
    ...
    const { destinationDir, destinationPath, actionType } =
      await resolveActionType(picked, undefined, true);
  ...
  const fieldPick = await prompt<{
    fields: ("category" | "chapter" | "tags" | "concepts")[];
  }>({
    type: "multiselect",
    name: "fields",
    message:
      "Which filters do you want to apply? (Select at least one. Use <space> to select, <a> to toggle all, <i> to invert selection):",
    choices: [
  ...
  if (fields.includes("tags")) {
    if (tags.length === 0) {
      ...
      const ans = await prompt<{ tags: string[] }>({
        ...
        name: "tags",
        message:
          "Select tag(s) (Press <space> to select, <a> to toggle all, <i> to invert selection):",
        choices: tags,
      });
  ...
  if (fields.includes("concepts")) {
    if (concepts.length === 0) {
      ...
      const ans = await prompt<{ concepts: string[] }>({
        ...
        name: "concepts",
        message:
          "Select concept(s) (Press <space> to select, <a> to toggle all, <i> to invert selection):",
        choices: concepts,
      });
  ...
  if (selectedCategory) {
    ...
    logger.info(
      `Found ${r.length} starter(s) matching category "${selectedCategory}".`,
    );
  ...
  if (selectedChapter) {
    ...
    logger.info(
      `Found ${r.length} starter(s) matching chapter "${selectedChapter}".`,
    );
    perFieldResults.push(r);
  }
  ...
  if (selectedTags.length > 0) {
    ...
    logger.info(
      `Found ${r.length} starter(s) matching tags "${selectedTags.join(", ")}".`,
    );
    perFieldResults.push(r);
  }
  ...
  if (selectedConcepts.length > 0) {
    ...
    logger.info(
      `Found ${r.length} starter(s) matching concepts "${selectedConcepts.join(", ")}".`,
    );
    perFieldResults.push(r);
  }
  ...
  logger.info(
    `Total starters selected after combining filters: ${combined.length}`,
  );
  logger.info(`Starters: ${combined.join(", ")}`);

  let dir: string | undefined = undefined;
  const { destinationDir, destinationPath, actionType } =
    await resolveActionType(combined, dir, true);
...
async function createStarters(
  plan: StarterCreatePlan,
  skipUI: boolean = false,
): Promise<void> {
  const { starterNames, destinationPath, destinationDir } = plan;
  const starterFrontendDir = path.join(
    destinationPath,
    config.starterFrontendDir || "ui",
  );
  ...
  logger.banner({
    ...
    rows: [
      {
        label: "Navigate",
        value: `cd ${quotePath(shortDestPath)}`,
        valueColor: "green",
      },
      { label: "Install", value: "npm install", valueColor: "green" },
      {
        label: "Start",
        value: "Start developing your FHE smart contracts!",
        valueColor: "cyan",
      },
    ],
  });
...
export async function runStarterCreate(opts: StarterCreateOptions) {
  ...
  if (fetchMode === "positional") {
    ...
    logger.info(
      `Creating starter(s) in ${plan.destinationPath} using mode: ${plan.mode}`,
    );
  ...
  if (fs.existsSync(plan.destinationPath)) {
    if (opts.force) {
      logger.warning(
        `Destination directory ${plan.destinationPath} already exists.Overwriting due to--force option.`,
      );
      ...
      logger.error(
        `Destination directory ${plan.destinationPath} already exists.Use--force to overwrite.`,
      );
      process.exit(1);
    }
  }
