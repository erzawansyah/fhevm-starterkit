...
import { isEmptyDir, run, quotePath } from "../helper/utils";
...
function ensureFrontendBuilt(templateDir: string, dryRun: boolean) {
  ...
  logger.info(
    `${dryRun ? "[dry-run] " : ""}Build frontend template karena dist belum tersedia.`
  );

  if (dryRun) return;

  run(`cd ${quotePath(templateDir)} && npm install`);
  run(`cd ${quotePath(templateDir)} && npm run build`);
}
...
export async function runStarterInit(
  ...
  logger.info(
    `${input.dryRun ? "[dry-run] " : ""}Copy base Hardhat template -> ${projectDir}`
  );
  if (!input.dryRun) {
  ...
  if (!input.skipUi) {
    ensureFrontendBuilt(FRONTEND_TEMPLATE_DIR, input.dryRun);
